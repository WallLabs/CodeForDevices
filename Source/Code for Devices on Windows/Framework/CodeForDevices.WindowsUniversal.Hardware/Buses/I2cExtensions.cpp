...
#pragma region Read
    ...
    uint8_t I2cExtensions::WriteReadByte(Windows::Devices::I2c::I2cDevice const& device, uint8_t writeData)
    {
        ...
        auto writeBuffer = single_threaded_vector<uint8_t>({ writeData });

        // Call overloaded method
        return WriteReadBytes(device, writeBuffer.GetView(), 1).GetAt(0);
    }

    uint8_t I2cExtensions::WriteReadByte(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& writeData)
    {
        // Call overloaded method
    ...
    Windows::Foundation::Collections::IVector<uint8_t> I2cExtensions::WriteReadBytes(Windows::Devices::I2c::I2cDevice const& device, uint8_t writeData, uint32_t size)
    {
        ...
        // Call overloaded method
        return WriteReadBytes(device, writeBuffer.GetView(), size);
    }

    Windows::Foundation::Collections::IVector<uint8_t> I2cExtensions::WriteReadBytes(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& writeData, uint32_t size)
    {
        // Create buffers
        auto readBuffer = array_view<uint8_t>(std::vector<uint8_t>(size));
        auto writeSize = writeData.Size();
        auto writeBuffer = array_view<const uint8_t>(std::vector<const uint8_t>(writeSize));

        // Call extended device method
        device.WriteRead(writeBuffer, readBuffer);

        // Return buffer
        return single_threaded_vector<uint8_t>(std::vector<uint8_t>{readBuffer});
    }
    ...
    bool I2cExtensions::WriteReadBit(Windows::Devices::I2c::I2cDevice const& device, uint8_t writeData, uint8_t mask)
    {
        ...
        // Call overloaded method and return result
        return WriteReadBit(device, writeBuffer.GetView(), mask);
    }

    bool I2cExtensions::WriteReadBit(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& writeData, uint8_t mask)
    {
        // Read byte
...
#pragma region Write
    ...
    void I2cExtensions::WriteBytes(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& data)
    {
        // Call extended device function
        device.Write(data);
    }
    ...
    /// <param name="data1">First part of data to write.</param>
    /// <param name="data2">Second part of data to write.</param>
    void I2cExtensions::WriteJoinByte(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& data1, uint8_t data2)
    {
        // Cast data to correct type
        auto data2Array = single_threaded_vector<uint8_t>({ data2 });

        // Call overloaded method
        WriteJoinBytes(device, data1, data2Array.GetView());
    }
    ...
    /// <param name="data1">First part of data to write.</param>
    /// <param name="data2">Second part of data to write.</param>
    void I2cExtensions::WriteJoinBytes(Windows::Devices::I2c::I2cDevice const& device, uint8_t data1, Windows::Foundation::Collections::IVectorView<uint8_t> const& data2)
    {
        // Cast data to correct type
        auto data1Array = single_threaded_vector<uint8_t>({ data1 });

        // Call overloaded method
        WriteJoinBytes(device, data1Array.GetView(), data2);
    }
    ...
    /// <param name="data1">First part of data to write.</param>
    /// <param name="data2">Second part of data to write.</param>
    void I2cExtensions::WriteJoinBytes(Windows::Devices::I2c::I2cDevice const& device, Windows::Foundation::Collections::IVectorView<uint8_t> const& data1, Windows::Foundation::Collections::IVectorView<uint8_t> const& data2)
    {
        // Join buffers
        auto addressLength = data1.Size();
        auto dataLength = data2.Size();
        auto buffer = std::vector<uint8_t>(0, addressLength + dataLength);
